...
class helpVies:
    ...
    def getOnlyMyImagesAndShared(images, user):
        ...
        for img in images:
            if user.name in img.accessNames:
                if (user.name == img.accessNames[0] and not img.isDeleted) or not user.name == img.accessNames[0]:
    ...
    def toJson(image):
        ...
        json = {'image_name': image.image.name, 'image_url': image.image.url, 'date': image.date, 'owner': image.owner,
                'userName': image.userName, 'accessNames': image.accessNames, 'isFavorite': image.isFavorite, 'isShared': (not image.accessNames[0] == image.owner), 'sdate': formatted_date, 'numfile': image.numfile, 'isDeleted': image.isDeleted}
        return json
