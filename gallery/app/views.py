...
def index1(request):
    if request.method == 'POST':
        ...
        return render(request, 'app/page1.html', {'form': formImage, 'rows': json.dumps(rows), 'rows_not_json': rows, 'user': user.name[0], 'color': user.color, 'usern': user.name, 'date': datetime.now().strftime('%d%m%Y'), 'datey': yesterday.strftime('%d%m%Y'), 'isHaveIdentify': json.dumps(isHas)})
