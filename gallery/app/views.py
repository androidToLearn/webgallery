...
def share_image(request):
    if request.method == 'POST':
        ...
        image = getObjectImageByNumFile(
            Image.objects.all(), myJson['numfile'])
...
def getObjectImageByNumFile(images, numFile):
    for image in images:
        if image.numfile == numFile:
            return image
    ...
    if request.method == 'POST':
        ...
        for img in images:
            ...
            Image.objects.create(image=img, userName=user.name,
                                 accessNames=[user.name], date=date, owner=user.name, numfile=len(Image.objects.all()))
